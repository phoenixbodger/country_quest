...
function GuessFlagForQuest({ targetCountry, countries, silhouetteGuessCount, capitalGuessCount, onPlayAgain }) {
  ...
  const totalGuesses = (silhouetteGuessCount || 0) + (capitalGuessCount || 0) + guessCount;
  ...
  return (
    <div>
      <div style={{ fontSize: '14px', color: '#a0aec0', marginBottom: '6px' }}>
        ...
        {capitalGuessCount > 0 && <> and its capital in {capitalGuessCount} {capitalGuessCount === 1 ? 'guess' : 'guesses'}</>}. Now guess its flag.
      </div>
      ...
      {gameWon && (
        <div style={{ color: '#a0aec0', fontSize: '14px', marginBottom: '12px' }}>
          🏁 Quest complete — {target.name.common}: {totalGuesses} total guesses across silhouette, capital & flag
        </div>
      )}
...
export default GuessFlagForQuest;