...
function NameTheCountry({
  ...
  const isInputDisabled = sessionActive ? (sessionRoundOver || guessesExhausted) : (gameWon || gameFailed);
  ...
  const handleGuessByCca3 = (cca3) => {
    if (sessionActive) {
      ...
      if (isCorrect) {
        const winFeat = features.find(f => f.properties.cca3 === cca3) || worldPolygons.find(f => f.properties?.cca3 === cca3);
      ...
      const clicked = features.find(f => f.properties.cca3 === cca3) || worldPolygons.find(f => f.properties?.cca3 === cca3);
  ...
  }, [popup, tried, sessionActive, sessionRoundOver, gameWon, guessesExhausted]);
  ...
  const handlePolygonClick = (polygon) => {
    console.log('[NameTheCountry] handlePolygonClick', polygon?.properties?.cca3, polygon?.properties?.name, 'sessionActive', sessionActive, 'sessionRoundOver', sessionRoundOver, 'gameWon', gameWon, 'guessesExhausted', guessesExhausted);
  ...
  const dotsData = useMemo(() => {
    ...
    // Debug: log all cca3 counts
    const cca3Counts = {};
    filtered.forEach(f => {
      const cca3 = f.properties.cca3;
      cca3Counts[cca3] = (cca3Counts[cca3] || 0) + 1;
    });
    const duplicates = Object.entries(cca3Counts).filter(([_, count]) => count > 1);
    if (duplicates.length > 0) {
      console.log('[DEBUG] Duplicate cca3 in worldPolygons:', duplicates);
    }
    
    // Debug: log Italy, Vatican, China, Hong Kong, Macau dots
    ['ITA', 'VAT', 'CHN', 'HKG', 'MAC'].forEach(cca3 => {
      const items = filtered.filter(f => f.properties.cca3 === cca3);
      if (items.length !== 1) {
        console.log(`[DEBUG] ${cca3} dots count:`, items.length, items.map(d => ({ cca3: d.properties.cca3, name: d.properties.name, latlng: d.properties.latlng })));
      }
    });
      ...
      <div style={{ marginTop: '16px', fontSize: '18px', fontWeight: 'bold' }}>
        {sessionActive ? (
          ...
          gameWon ? (
            ...
            <span style={{ color: '#fc8181' }}>❌ All wrong choices selected — The answer was {target.properties.name}{targetCountryObj?.flag ? ` ${targetCountryObj.flag}` : ''}</span>
          ) : (
